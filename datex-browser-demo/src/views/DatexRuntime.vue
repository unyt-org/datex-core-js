            ...
            <div>
                ...
                <div class="mt-4">
                    <ul class="flex flex-col gap-2 w-2xl">
                        <li v-for="inter in comHubStatus.interfaces?.toSorted((a: any, b: any) => a.properties.interface_type.localeCompare(b.properties.interface_type))"
                            :key="inter.id" class="bg-gray-900 rounded-md p-2">
                            <div>
                                <b>{{ inter.properties.interface_type }}/{{ inter.properties.channel }}</b><span
                                    v-if="inter.properties.name"> ({{ inter.properties.name }})</span>
                            </div>
                            <ul v-for="socket in inter.sockets.sort((a: any, b: any) => {
                                ...
                                <li :key="socket.uuid" :title="socket.uuid">
                                    <div>
                                        ...
                                        <span v-if="socket.properties.is_direct"
                                            class="bg-cyan-600 px-1.5 py-0.5 rounded-md ml-2">direct</span>
                                        <span v-if="socket.properties.distance !== undefined"
                                            class="bg-gray-600 px-1.5 py-0.5 rounded-md ml-2">distance: {{
                                            socket.properties.distance }}</span>
                                    </div>
                                </li>
                            </ul>
                        </li>
