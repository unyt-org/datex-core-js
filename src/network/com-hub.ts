...
export class ComHub {
    ...
    static #interfaceImpls = new Map<keyof GlobalInterfaceImpls, typeof ComInterfaceImpl<unknown>>();
    static #interfaceImplsByClass = new Map<typeof ComInterfaceImpl<unknown>, keyof GlobalInterfaceImpls>();
    ...
    static registerInterfaceImpl<N extends keyof GlobalInterfaceImpls>(interfaceType: N, impl: GlobalInterfaceImpls[N] & typeof ComInterfaceImpl<unknown>) {
    ...
    async createInterface<N extends keyof GlobalInterfaceImpls, T = GlobalInterfaceImpls[N]>(
        interfaceType: N,
        setupData: T extends typeof ComInterfaceImpl<infer P> ? P : never
    ...
    async createInterface(
        ...
        const type = typeof interfaceType === "string" ? interfaceType : ComHub.#interfaceImplsByClass.get(interfaceType);
        if (type === undefined) {
            throw new Error(`Interface implementation for ${(interfaceType as typeof ComInterfaceImpl).name} not registered.`);
        }
        ...
        const uuid = await this.#jsComHub.create_interface(type, JSON.stringify(setupData));
        const impl = new (implClass as (new(uuid: string, setupData: unknown, comHub: JSComHub) => ComInterfaceImpl<unknown>))(uuid, setupData, this.#jsComHub);
        await impl.init?.();
