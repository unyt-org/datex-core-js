...
pub trait WebRTCTrait {
    ...
    fn remote_endpoint(&self) -> Endpoint {
        self.get_info().borrow().endpoint.clone()
    }
...
pub struct WebRTCJSInterfaceNew {
    pub info: Rc<RefCell<WebRTCCommon>>,
...
impl WebRTCTrait for WebRTCJSInterfaceNew {
    fn new(peer_endpoint: impl Into<Endpoint>) -> Self {
        WebRTCJSInterfaceNew {
            info: Rc::new(RefCell::new(WebRTCCommon::new(peer_endpoint))),
    ...
    async fn handle_create_offer(&self) -> Result<Vec<u8>, WebRTCError> {
        if let Some(peer_connection) = self.peer_connection.as_ref() {
            ...
            JsFuture::from(sld_promise)
                .await
                .map_err(|_| WebRTCError::InvalidSdp)?;
            Ok(serialize::<String>(&offer_sdp).unwrap())
        } else {
    ...
    async fn handle_set_local_description(
        ...
        if let Some(peer_connection) = self.peer_connection.as_ref() {
            let sdp = deserialize::<String>(&sdp)
                .map_err(|_| WebRTCError::InvalidSdp)?;
            let description = RtcSessionDescriptionInit::new(RtcSdpType::Offer);
            description.set_sdp(&sdp);
            JsFuture::from(peer_connection.set_local_description(&description))
                .await
                .unwrap();
            Ok(())
        } else {
    ...
    async fn handle_create_answer(&self) -> Result<Vec<u8>, WebRTCError> {
        if let Some(peer_connection) = self.peer_connection.as_ref() {
            ...
            let answer_sdp = Reflect::get(&answer, &JsValue::from_str("sdp"))
                ...
                .as_string()
                .unwrap();
            Ok(serialize::<String>(&answer_sdp).unwrap())
        } else {
            error!("Peer connection is not initialized");
            Err(WebRTCError::ConnectionError)
        }
